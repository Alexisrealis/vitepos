...
class Pos_Order_Api extends API_Base {
			...
			if ( ! empty( $cashdrawer ) ) {
				...
				foreach ( $payment_list as $payment ) {
					if ( 'C' == $payment['type'] ) {
						...
						if($amount>0.0) {
							Mapbd_Pos_Cash_Drawer::add_order( $this->get_current_user_id(), $amount, $order->get_id(),
								$outlet_id, $counter_id );
						}else{
							Mapbd_Pos_Cash_Drawer::add_order( $this->get_current_user_id(), doubleval( $payment['amount'] ), $order->get_id(),
								$outlet_id, $counter_id );
	...
	public function check_discount_limit($subtotal,$user_discount,$discounts=[]){
