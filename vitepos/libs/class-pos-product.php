...
class POS_Product {
	 ...
	 *
	 * @param \WP_Term[]|int[]|string[]|string|\WP_Error $category
	 ...
	 * @return array
	 */
	public static function get_categories( $is_parent = false ) {
		$orderby    = 'name';
		$order      = 'asc';
		$cat_args   = array(
			'orderby'    => $orderby,
			'order'      => $order,
			'hide_empty' => $is_parent,
		);
		if (!$is_parent)
		{
			$cat_args['parent'] = 0;
	...
	public static function get_product_data( $product, $add_ctg_id = false, $is_no_variable = false ) {
		...
		$pos_product->stock_quantity = $product->get_stock_quantity();
		$pos_product->sale_price     = $product->get_sale_price() ? $product->get_sale_price() : null;
		...
		if ( $add_ctg_id ) {
			$pos_product->categories = wc_get_object_terms( $product->get_id(), 'product_cat', 'term_id' );
		} else {
			$pos_product->categories = wc_get_object_terms( $product->get_id(), 'product_cat', 'slug' );
		}
		...
		$pos_product->purchase_cost  = $product->get_meta( '_vt_purchase_cost' );
	...
	public static function get_product_variation_data( $product ) {
		...
		$pos_product->sku           = $product->get_sku();
		$pos_product->purchasable   = 'Y';
		$pos_product->purchase_cost = $product->get_meta( 'purchase_cost' ) ? $product->get_meta( 'purchase_cost' ) : '0.00';
	...
	private static function get_variation_data( $product ) {
		...
		foreach ( $product->get_children() as $child_id ) {
			...
			$variation_obj->sale_price    = $variation->get_sale_price() ? $variation->get_sale_price() : null;
			$variation_obj->regular_price = $variation->get_regular_price();
			$variation_obj->price         = $variation->get_price();
			...
			$variation_obj->stock_quantity      = $variation->get_stock_quantity();
			...
			$variation_obj->barcode             = self::get_barcode_of_product( $variation );
			$variation_obj->purchase_cost       = $variation->get_meta( '_vt_purchase_cost' );
